...
--2021/04/14 Y.Nishikawa DEL 出荷指示詳細は引当ファンクションで処理しているため、二重計上↓↓
----TableF
...
--D_JuchuuMeisai
UPDATE  A
--2021/04/14 Y.Nishikawa CHG この時点では受注明細の出荷指示済数に今回出荷指示数を更新していないので、分納時は完了区分が完了にならない↓↓
...
--2021/04/14 Y.Nishikawa ADD 条件不足（条件に行番号がないので、受注明細数分膨れて計算される）↓↓
