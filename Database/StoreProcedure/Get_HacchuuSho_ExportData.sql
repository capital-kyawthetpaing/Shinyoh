...
--SELECT
--A.SiiresakiCD AS 'SiiresakiCD'
--,MAX(A.SiiresakiName) AS 'SiiresakiName'
--,C.Model_No AS 'ModelNo'
--,MAX(C.Model_Name) AS 'ModelName'
--,MAX(C.FOB) AS 'FOBPRICE'
--,B.ColorNO AS 'ColorNO'
--,MAX(M_MultiPorpose_Color.Char1) as 'JAPANColor'
--,MAX(M_MultiPorpose_Color.Char3) as 'KOREAColor'
--,MAX(C.Shipping_Place) as 'Shippingplace'
--,MAX(C.ShouhinImage)as 'IMAGE'
--,SUM(B.HacchuuSuu) as 'Pairs'
--,SUM(B.HacchuuSuu) * MAX(C.FOB) as 'Amount'
--INTO #WK_HacchuuMeisai
--FROM D_Hacchuu A
...
--WHERE(@JuchuuNO1 is null or(B.JuchuuNO>=@JuchuuNO1))
...
--AND (@FW is null or (C.SeasonFW=@FW))
--and A.HacchuushoHakkouFLG=@Rdo_Type
----and SUM(B.HacchuuSuu)<>0
--Group by A.SiiresakiCD,C.Model_No,B.ColorNO,B.SizeNO


declare @Size as nvarchar(200)= N'23.0,23.5,24.0,24.5,25.0,25.5,26.0,26.5,27.0,27.5,28.0';
SELECT B.ColorNO,
A.SiiresakiCD,
...
(	SELECT 
	...
	LEFT OUTER JOIN (SELECT * FROM M_MultiPorpose WHERE ID = 104) M_MultiPorpose_Color ON M_MultiPorpose_Color.[Key] = B.ColorNO
	WHERE
	--B.SizeNO in (select val from split(@Size,',') )and
